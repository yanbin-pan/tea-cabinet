...
async function persist(collection) {
  ...
  // even if the page is refreshed before the async store below finishes.
  try { if (window.localStorage) window.localStorage.setItem(STORAGE_KEY, json); } catch (e) {}
  // Durable async store (Claude artifact storage), if available.
  try {
...
async function hydrate() {
  // Prefer the durable async store; fall back to the synchronous mirror if it's
  // empty or unavailable (covers the refresh-race where the async write was lost).
  try {
